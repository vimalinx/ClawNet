...
## Vimalinx Server stack (server + plugin + Android app)

This repo also ships the Vimalinx Server channel stack:
- Server: `server`
- Plugin: `plugin`
- Android app (Vimagram): `app`

Deployment flow (high level):
1) Run the Vimalinx Server (Node 22+). Configure users + invite codes.
2) Install the plugin (`clawdbot plugins install ./plugin`) and run
   `clawdbot onboard` to select server + paste the host token.
3) Build Vimagram, log in to the server, and generate the host token from the
   Account screen.

Details live in each subproject README.

## Vimalinx CLI quickstart

Use this when you want to stand up the server + plugin from a local checkout.
